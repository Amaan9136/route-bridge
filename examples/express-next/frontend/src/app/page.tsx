...
/** createApiClient */
const api = createApiClient({ baseUrl: "http://localhost:5000/api", debug: true });
...
export default function DemoPage() {
  ...
  return (
    <main className="font-mono max-w-2xl mx-auto my-8 px-4">
      <h1 className="text-2xl font-bold text-blue-600 mb-2">🌉 route-bridge demo</h1>
      <p className="text-gray-500 mb-6">
        All API calls below use the <strong>auto-generated</strong>{" "}
        <code className="bg-gray-100 px-1 rounded">api.*</code> client — no handwritten fetch wrappers.
      </p>
      {error && (
      ...
      <Section title="GET /posts">
        ...
        <Btn
          onClick={async () => { const d = await run("posts", () => api.listPosts({ query: { page: postsPage, limit: postsLimit } })); if (d) setPosts(d.posts as Post[]); }}
...
function Btn({ onClick, disabled, loading, danger }: { onClick: () => void; disabled: boolean; loading: boolean; danger?: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`mt-2 px-5 py-2 text-white rounded font-mono text-sm transition-opacity disabled:opacity-50 ${danger ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-blue-700"}`}
    >
      {loading ? "Loading…" : "Run"}
    </button>
  );
