 ...
 * Source: Flask backend via route-bridge.manifest.json
...
export type getGreetingResponse = { message: string };
...
export type getUserByIdResponse = { id: string; name: string; email: string };
...
export type listPostsResponse = { posts: unknown; total: number; page: number };
...
export type deletePostResponse = { success: boolean };
...
class RouteBridgeClient {
  ...
  private async _call<T>(params: CallParams, attempt = 0): Promise<T> {
    ...
    try {
      ...
      if (attempt < this.options.retries) return this._call<T>(params, attempt + 1);
    ...
    if (this.options.debug) console.debug(`[route-bridge] ← ${response.status}`, data);
  ...
  async getGreeting(options: getGreetingOptions = {}): Promise<getGreetingResponse> {
    return this._call({ method: "GET", path: "/greeting", query: options.query, headers: options.headers });
  }

  async getUserById(options: getUserByIdOptions): Promise<getUserByIdResponse> {
    return this._call({ method: "GET", path: `/users/${options.params.id}`, headers: options.headers });
  }

  async createUser(options: createUserOptions = {}): Promise<createUserResponse> {
    return this._call({ method: "POST", path: "/users", body: options.body, headers: options.headers });
  }

  async listPosts(options: listPostsOptions = {}): Promise<listPostsResponse> {
    return this._call({ method: "GET", path: "/posts", query: options.query, headers: options.headers });
  }

  async deletePost(options: deletePostOptions): Promise<deletePostResponse> {
    return this._call({ method: "DELETE", path: `/posts/${options.params.id}`, headers: options.headers });
  }
}
